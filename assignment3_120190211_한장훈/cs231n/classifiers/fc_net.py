...
def nomal_distribution_random(sigma,dim1,dim2):
    matrix=[]
    for i in range(dim1):
        row=[]
        for j in range(dim2):
            row.append(random.normalvariate(0.0, sigma))
        matrix.append(np.array(row,dtype=np.float64))
    return np.array(matrix)

def squaresum(nparr):
    nplist=list(nparr)
    squaresum=0
    for row_list in nplist:
        squarelist=map(lambda x: x ** 2, row_list)
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        '''
        self.params['W1']=nomal_distribution_random(weight_scale,input_dim,hidden_dim)
        self.params['b1']=np.array([0.]*hidden_dim)
        self.params['W2'] =nomal_distribution_random(weight_scale, hidden_dim, num_classes)
        self.params['b2'] = np.array([0.]*num_classes)
    ...
    def loss(self, X, y=None):
        ...
        print("loss",loss)
        '''
        loss += 0.5*self.reg*(squaresum(self.params['W1']) + squaresum(self.params['W2']))
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers - 1):
            ...
            '''
            self.params['W' + str(i+1)]=nomal_distribution_random(weight_scale,input_dim,hidden_dims[i])
            self.params['b' + str(i+1)] = np.array([0.]*hidden_dims[i])
            ...
            if self.use_batchnorm:
                ...
                '''
                self.params['beta' + str(i+1)] = np.array([0.]*hidden_dims[i])
                self.params['gamma' + str(i+1)] =np.array([1.]*hidden_dims[i])
    ...
    def loss(self, X, y=None):
        ...
        loss += 0.5*self.reg*(squaresum(self.params['W'+str(self.num_layers)]))
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            loss += 0.5 * self.reg * (squaresum(self.params['W' + str(i)]))
            ## 문제 4-1. end block #############################################################################
