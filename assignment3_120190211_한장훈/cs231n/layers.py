...
import statistics
import random
def reshape(nparr, shape):
    
    new_np=[0.]*shape[0]
    #print("dd",nparr.shape)
    if nparr.ndim == 4 and shape[1]==-1:
        shape[1]=len(nparr[0])*len(nparr[0][0])*len(nparr[0][0][0])
        for i,rest in enumerate(nparr):
            new_np[i]=np.array([element for list2 in rest for list1 in list2 for element in list1])
    
    if nparr.ndim ==3 and shape[1]==-1:
        shape[1]=len(nparr[0])*len(nparr[0][0])
        
        for i,rest in enumerate(nparr):
            new_np[i]=np.array([element for list2 in rest for element in list2])
    
    #print(nparr.shape,shape)
    if nparr.ndim==2:
        if len(shape)==2:
            return nparr
        for i,rest in enumerate(nparr):
            ...
            for j in range(shape[1]):
                ...
                list1.append(rest[start:end])
            new_np[i]=np.array(list1)
    #print(new_np.shape)
    
    return np.array(new_np)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        for row in x.T:
            ...
            #print(row)
            sample_mean_list.append(statistics.mean(row))
            sample_var_list.append(statistics.pvariance(row))
        sample_mean=np.array(sample_mean_list)
        sample_var=np.array(sample_var_list)
        ...
        sqrt_list=[]
        for i in sample_var+eps:
            sqrt_list.append(math.sqrt(i))
        normalized_data = (x - sample_mean) / np.array(sqrt_list)
        
        #print(sample_var + eps)
        ...
        cache = {
            ...
            'normalized_data': normalized_data,
            'gamma': gamma,
            'ivar': 1./np.array(sqrt_list),
            'sqrtvar': np.array(sqrt_list),
        }
        ...
        running_var_list=[]
        for i in running_var + eps:
            running_var_list.append(math.sqrt(i))
        #print(np.array(running_var_list))
        #print(np.sqrt(running_var+eps))
        out = (gamma / (np.array(running_var_list)) * x) + (beta - (gamma*running_mean)/np.array(running_var_list))
...
def batchnorm_backward(dout, cache):
    ...
    dbeta_list=[]
    for i in dout.T:
        dbeta_list.append(sum(i))
    dbeta=np.array(dbeta_list)
    
    dgamma_list=[]
    for i in (dout * normalized_data).T:
        dgamma_list.append(sum(i))
    dgamma=np.array(dgamma_list)
    ...
    divar_list=[]
    for i in (dxhat*x_minus_mean).T:
        divar_list.append(sum(i))
    divar=np.array(divar_list)
    # Step4
    dsqrtvar = divar * (-1/sqrtvar**2)
    # Step5
    dvar = dsqrtvar * 0.5 * (1/sqrtvar)
    # Step6
    dout_ones_like=np.array([np.array([1.]*D)for _ in range(N)])
    dsq = (1/N)*dvar*dout_ones_like
    ...
    dmu_list=[]
    for i in (dxmu1 + dxmu2).T:
        dmu_list.append(sum(i))
    dmu = -1*np.array(dmu_list)
    # Step9
    #dx2 = (1/N)*dmu*np.ones_like(dout)
    dx2 = (1/N)*dmu*dout_ones_like
    # Step10
    dx = dx2 + dx1
    ## 문제 8. end block ###############################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    squarelist=list(map(lambda x: x ** 2, ivar))
    ...
    dx =(1 / N) * gamma * 1/sqrtvar * ((N * dout) - axis0sum(dout) - (x_minus_mean) * np.array(squarelist) * axis0sum(dout * (x_minus_mean)))
    ## 문제 9. end block ###############################################################################
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        for i in range(a):
            mask_list_row=[]
            for j in range(b):
                mask_list_row.append(random.random()>=p)
            mask_list.append(np.array(mask_list_row))
