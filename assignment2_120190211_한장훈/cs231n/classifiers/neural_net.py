...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #print(relu_1_activation.shape)
    #print(W2.shape)
    # FC2 layer.
    #fc2_activation = np.dot(relu_1_activation, W2) + b2
    #fc2_activation=relu_1_activation@W2+b2
    fc2_activation=self.custom_dot(relu_1_activation,W2)+b2
    ...
    lse_shift_scores=np.array([0]*shift_scores.shape[0],dtype=np.float64)
    for i,shift_score in enumerate(shift_scores):
        sum_shift_score=0
        for each_score in shift_score:
            sum_shift_score+=math.exp(each_score)
        lse_shift_scores[i]=math.log(sum_shift_score)
    #print(lse_shift_scores) 
    #print(np.log(np.sum(np.exp(shift_scores), axis=1)))
    loss = -correct_class_scores +lse_shift_scores    
    loss = sum(loss)
    loss /= N
    ...
    dW2 = self.custom_dot(relu_1_activation.T,dSoft)
    ...
    dx2 = self.custom_dot(dSoft,W2.T)
    ...
    dW1 = self.custom_dot(X.T,dRelu1)
    ...
    grads['b1']=sum(db1)
    # 문제 3-5: 위 구문(line: 154)을 numpy lib를 사용하지 않고 numpy lib를 사용한 결과와 동일하게 동작하도록 작성
  ...
  def predict(self, X):
    ...
    losses=self.loss(X)
    #print(losses)
    y_pred=np.array([0]*losses.shape[0],dtype=np.float64)
    for idx,loss in enumerate(losses):
        f = lambda i: loss[i]
        y_pred[idx]=max(range(len(loss)), key=f)
